...
class IPSL_DCPP(torch.utils.data.Dataset):
    def __init__(self,domain,lead_time_months,generate_statistics=False):
        self.domain = domain
        ...
        self.surface_means_test = np.expand_dims(np.load(f'{work}/ipsl_dcpp/data/surface_means_test.npy'),axis=(1,2))
        self.surface_stds_test = np.expand_dims(np.load(f'{work}/ipsl_dcpp/data/surface_stds_test.npy'),axis=(1,2))
        self.plev_means_test = np.expand_dims(np.load(f'{work}/ipsl_dcpp/data/plev_means_test.npy'),axis=(1,2,3))
        self.plev_stds_test = np.expand_dims(np.load(f'{work}/ipsl_dcpp/data/plev_stds_test.npy'),axis=(1,2,3))
        self.generate_statistics=generate_statistics
        self.timestamps = []
        self.id2pt_path = f'{work}/ipsl_dcpp/data/{domain}_id2pt.pkl'
    ...
    def preprocess(self, clim,clim_next):
        ...
        if(not self.generate_statistics):
            if(self.domain == 'train' or self.domain == 'val'):
            ...
            elif(self.domain == 'test'):
                input_surface_variables = (input_surface_variables - self.surface_means_test) / self.surface_stds_test
                input_plev_variables = (input_plev_variables - self.plev_means_test) / self.plev_stds_test
               # input_depth_variables = (input_depth_variables - self.input_means) / self.input_stds
                target_surface_variables = (target_surface_variables - self.surface_means_test) / self.surface_stds_test
                target_plev_variables = (target_plev_variables - self.plev_means_test) / self.plev_stds_test                
    ...
    def denormalize(self, pred, batch):
        ...
        if(self.domain == 'test'):
            denorm_level = lambda x: x*torch.from_numpy(self.plev_stds_test).to(device) + torch.from_numpy(self.plev_means_test).to(device)
            denorm_surface = lambda x: x*torch.from_numpy(self.surface_stds_test).to(device) + torch.from_numpy(self.surface_means_test).to(device)
        else:
