...
class IPSL_DCPP(torch.utils.data.Dataset):
    def __init__(self,domain,lead_time_months,generate_statistics=False):
        ...
        self.land_mask = np.expand_dims(np.load(f'{work}/ipsl_dcpp/data/land_mask.npy'),axis=(0,1))
        ...
        self.depth_means = np.expand_dims(np.load(f'{work}/ipsl_dcpp/data/depth_means.npy'),axis=(1,2,3))
        self.depth_stds = np.expand_dims(np.load(f'{work}/ipsl_dcpp/data/depth_stds.npy'),axis=(1,2,3))
