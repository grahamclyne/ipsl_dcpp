...
for count in range(1000):
  ...
  #  i_mean,i_std = z_normalize(surface.numpy(),(1,2))
    i_mean,i_std = z_normalize(surface.numpy(),(0,1))

   # t_mean,t_std = z_normalize(plev.numpy(),(1,2,3))
    #d_mean,d_std = z_normalize(depth.numpy(),(1,2,3))
    d_mean,d_std = z_normalize(depth.numpy(),(0,1,2))
...
depth_means_out = np.nanmean(np.array(depth_means),axis=0)
depth_stds_out = np.nanmean(np.array(depth_stds),axis=0)
np.save('data/single_var_surface_means',surface_means_out)
np.save('data/single_var_surface_stds',surface_stds_out)
#np.save('data/plev_means',plev_means_out)
#np.save('data/plev_stds',plev_stds_out)
#np.save('data/depth_means',depth_means_out)
#np.save('data/depth_stds',depth_stds_out)
